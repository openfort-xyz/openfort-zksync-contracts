...
// required to validate session key in the validateTransaction hook
    ...
    OpenfortEvents
{
    ...
    bytes32 internal constant OF_MSG_TYPEHASH =
        0x57159f03b9efda178eab2037b2ec0b51ce11be0051b8a2a9992c29dc260e4a30;
    // salt 0x666
    address internal constant BATCH_CALLER_ADDRESS =
        0x7219257B57d9546c1BC0649617d557Db09C92D23;
    ...
    function __BaseOpenfortAccount_init(
    ...
    function _validateSignature(
        ...
        if (
            signerAddress == owner() ||
            isValidSessionKey(signerAddress, _to, _data)
        ) {
            magic = EIP1271_SUCCESS_RETURN_VALUE;
        }
    }

    function _recoverECDSAsignature(
        bytes32 _hash,
        bytes memory _signature
    ...
    function isValidSessionKey(
        ...
        ITimestampAsserter(timestampAsserter).assertTimestampInRange(
    ...
    function _validateSessionKeyCall(
        SessionKeyStruct storage _sessionKey,
        address _to
    ...
    function isValidSignature(
        ...
        if (
            sessionKey.validUntil == 0 ||
            sessionKey.validAfter > block.timestamp ||
    ...
    function _executeTransaction(Transaction calldata _transaction) internal {
        ...
        require(
            to != address(DEPLOYER_SYSTEM_CONTRACT),
            "Deployment from smart account not supported"
        );
    ...
    function _executeDelegatecall(
        ...
        assembly {
            ...
            if mod(o, 32) {
                o := and(add(o, 31), not(31))
            }
            mstore(0x40, o)
            // Forwards the `data` to `delegate` via delegatecall.
            if iszero(
                delegatecall(gas(), delegate, ptr, callData.length, 0, 0)
            ) {
